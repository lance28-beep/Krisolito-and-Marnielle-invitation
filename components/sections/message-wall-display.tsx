            ...
            <CardContent className="p-2.5 sm:p-3 md:p-4 lg:p-5">
              <div className="flex justify-between items-start mb-2 sm:mb-3 md:mb-4">
                <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4">
                  <Skeleton className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-[#324D3E]/35 via-[#324D3E]/30 to-[#324D3E]/30" />
                  <div className="space-y-1.5 sm:space-y-2">
                    <Skeleton className="h-3 w-20 sm:w-24 md:w-32 bg-[#324D3E]/30" />
                    <Skeleton className="h-2.5 w-16 sm:w-20 md:w-24 bg-[#324D3E]/20" />
                  </div>
                </div>
                <div className="flex gap-1.5 sm:gap-2">
                  <Skeleton className="w-3 h-3 sm:w-4 sm:h-4 rounded bg-[#324D3E]/25" />
                  <Skeleton className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded bg-[#324D3E]/15" />
                </div>
              </div>
              <Skeleton className="h-12 sm:h-14 md:h-16 w-full bg-gradient-to-r from-[#324D3E]/14 via-[#324D3E]/10 to-[#324D3E]/16 rounded-lg" />
            </CardContent>
          </Card>
        ))}
      </div>
    ...
    return (
      <div className="text-center py-6 sm:py-10 md:py-14 lg:py-16 xl:py-20 px-2 sm:px-4">
        <div className="relative inline-block mb-4 sm:mb-5 md:mb-6 lg:mb-8">
          <div className="absolute inset-0 bg-gradient-to-r from-[#324D3E]/45 to-[#324D3E]/30 rounded-full blur-xl scale-150 animate-pulse-slow" />
          <div className="relative w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 lg:w-18 lg:h-18 bg-gradient-to-br from-[#324D3E] via-[#324D3E] to-[#324D3E] rounded-full flex items-center justify-center mx-auto shadow-lg">
            <MessageCircle className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 lg:h-10 lg:w-10 text-white" />
          </div>
          {/* Outer decorative rings */}
          <div className="absolute -inset-2 sm:-inset-3 rounded-full border-2 border-[#324D3E]/25 animate-ping"></div>
          <div className="absolute -inset-1.5 sm:-inset-2 rounded-full border border-[#324D3E]/40"></div>
        </div>
      ...
      {visibleMessages.map((msg, index) => (
        <Card
          ...
          style={{
            ...
            boxShadow: '0 4px 18px rgba(50,77,62,0.25), 0 2px 8px rgba(50,77,62,0.18)'
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.boxShadow = '0 8px 26px rgba(50,77,62,0.35), 0 4px 12px rgba(50,77,62,0.25)';
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.boxShadow = '0 4px 18px rgba(50,77,62,0.25), 0 2px 8px rgba(50,77,62,0.18)';
          }}
        >
          {/* Enhanced card background effects */}
          <div className="absolute inset-0 bg-gradient-to-br from-[#324D3E]/18 via-transparent to-[#BCCFC0]/18 opacity-70 group-hover:opacity-95 transition-opacity duration-300" />
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#324D3E]/55 via-[#324D3E]/60 to-[#324D3E]/55 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#BCCFC0]/22 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
          
          {/* Simple inner border */}
          <div className="absolute inset-2 sm:inset-3 rounded-xl pointer-events-none">
            <div className="absolute inset-0 rounded-xl border border-[#324D3E]/15 group-hover:border-[#324D3E]/35 transition-colors duration-300" />
          </div>
          ...
          <CardContent className="relative p-2 sm:p-2.5 md:p-3 lg:p-3.5">
            <div className="flex justify-between items-start mb-1.5 sm:mb-2 md:mb-2.5">
              <div className="flex items-center space-x-1.5 sm:space-x-2 md:space-x-2.5">
                <div className="relative">
                  <div className="w-7 h-7 sm:w-9 sm:h-9 md:w-11 md:h-11 bg-[#324D3E] rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-lg ring-2 ring-white/70">
                  ...
                  {/* Subtle avatar glow */}
                  <div className="absolute -inset-1 rounded-full bg-[#324D3E]/30 blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline gap-1 sm:gap-1.5">
                    <h4 className="font-lora text-[#324D3E] text-xs sm:text-sm md:text-base font-semibold truncate group-hover:text-[#324D3E] transition-colors duration-300">
                      {msg.name}
                    </h4>
                    <span className="text-[9px] sm:text-[10px] md:text-xs text-[#324D3E]/70 font-lora truncate">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                <Heart className="h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5 text-[#324D3E]/80 fill-[#324D3E]/20 group-hover:fill-[#324D3E]/45 group-hover:text-[#324D3E] transition-all duration-300 group-hover:scale-110" />
                <Sparkles className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4 text-[#324D3E]/85 group-hover:text-[#324D3E] transition-all duration-300 group-hover:scale-110 group-hover:rotate-12" />
              </div>
            </div>
            ...
            <div className="mt-1.5 sm:mt-2 md:mt-3 flex items-center justify-between">
                <div className="flex items-center gap-0.5 sm:gap-1 text-[9px] sm:text-[10px] text-[#324D3E]/65">
                  <div className="w-1 h-1 rounded-full bg-[#324D3E]/80" />
                  <div className="w-1 h-1 rounded-full bg-[#324D3E]/80" />
                  <div className="w-1 h-1 rounded-full bg-[#324D3E]/80" />
              </div>
                <div className="w-14 sm:w-16 h-0.5 bg-gradient-to-r from-transparent via-[#324D3E]/70 to-transparent group-hover:via-[#324D3E]/85 transition-all duration-300" />
            </div>
          </CardContent>
        </Card>
      ))}
